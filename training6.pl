...
% where G is input for goal (stength(str), size(size), or endurance(end))
...
% Protein, Carb, and Fats are numeric values that should each be (percentages / 100) between 0 and 1 and together add up to 1

%calRatio(Calories, Protein, Carb, Fat, Ratio) is true if Value is a list of Protein, Carb, and Fat in grams is required to achieve daily Calories
calRatio(Calories, Protein, Carb, Fat, Ratio) :-
    0 < Protein,
    Protein < 1,
    0 < Carb,
    Carb < 1,
    0 < Fat,
    Fat < 1,
    1.0 is Protein+Carb+Fat,
    Pcal is Calories*Protein,
    Ccal is Calories*Carb,
    Fcal is Calories*Fat,
    Pgrams is Pcal/4,
    Cgrams is Ccal/4,
    Fgrams is Fcal/9,
    append([Pgrams],[Cgrams],X),
    append(X,[Fgrams],Ratio).
